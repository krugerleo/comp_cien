...
double* gaussSeidel(double** A, double* b, double* x1,int n){
	...
	do{
		...
		for(i=0; i < n; i++){
			...
			somatorio2 = 0.0;
			for(j = 0; j < i; j++){
				somatorio1 += (A[i][j]*x[kAtual][j]);
			}
			for(j = i+1; j < n; j++){
				somatorio2 += (A[i][j]*x[kAntigo][j]);
			}
			x[kAtual][i] = (b[i] - somatorio1 - somatorio2) / A[i][i] ;
		}
		/*
		printf("--Atual-- %d\n",kAtual);
...
int main(){
	...
	puts("----------Gauss Seidel-----------");
	double* y = gaussSeidel(A,b,x,N);
	puts("-------------------------");
	puts("Apos Gauss Seidel:");
	puts("x:");
	imprimeVetor(y);
	return 0;
}
